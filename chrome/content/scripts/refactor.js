...
Zotero.ShortDOI = new function () {
	...
	this.setCheck = function () {
	    ...
	    var autoshortvalue = getPref("autoshort");
	...
	*/
	 ...
	 */
	this.getDOIs = function (items, operation) {
	...
	*/
	...
	function _addItem(item) {
		for (let row of _rows) {
			if (row.id === item.id) {
				if (row.status > Zotero.ShortDOI.ROW_PROCESSING) {
					_deleteRow(row.id);
					break;
				}
				return null;
	...
	/**
	 * Processes the item and saves the DOI and tags
	 * @param itemID
	 * @return {Promise}
	 */
